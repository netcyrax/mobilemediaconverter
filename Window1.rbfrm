...
#tag Events lstIn
	...
	#tag Event
		Function CellTextPaint(g As Graphics, row As Integer, column As Integer, x as Integer, y as Integer) As Boolean
		  ...
		  if column = 0 then
		    // scale and redraw the thumbnail in the list
		    if (scrs(row)<>nil) then g.DrawPicture(scrs(row),2,2,60,60,0,0,scrs(row).Width,scrs(row).Height)
